...
import com.example.mydb.db.helper.DBHelper;
import com.example.mydb.model.Category;
import com.example.mydb.model.Product;
...
public class DB {
    ...
    public Cursor getCursorCategoria(){
        return dbHelper.getReadableDatabase().rawQuery("select * from categoria",null );
    }
    //consulta multitabla
    ...
    public Cursor getCursorProducto(String id_categoria){
        return dbHelper.getReadableDatabase().rawQuery(
                "select p.id_producto ,p.nombre ,p.descripcion ,p.id_categoria " +
                        ...
                        "p.id_categoria=?", new String[]{id_categoria});
    }
    // Ir a crear el modelo
    ...
    //Ir al main activity a crear la vista
}
